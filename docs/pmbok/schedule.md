...
**現状の読み方**: フェーズの**順序・範囲**(SPI=1.0)は計画通りだが、**コスト効率**(CPI=0.5)は見積りの半分しか出ておらず、このペースが続くと完成時には当初16時間予算の**倍(32時間)**かかる計算になる。[risk-register.md](risk-register.md) R1が既に「スケジュール超過は承知の上」としていた通りの傾向が、数値としても裏付けられた形。

**方針転換([change-log.md](change-log.md) C12)**: このCPI低下を「見積りを外した失敗」として追いかけるのではなく、「時間が固定・品質(SOLID比較・PMBOK実体験)は死守という前提のもとで、スコープを絞る判断を行うべきタイミングを示すシグナル」として読み替えることにした。EACの32時間という数値自体を予算超過の証拠として問題視するのではなく、**残り時間内に収まるようフェーズ4以降のスコープをMust(MVP)/Should(余剰があれば着手)に仕分ける**判断材料として使う([risk-register.md](risk-register.md) R1の具体的な仕分けを参照)。次フェーズ(フロントエンド実装、Must部分)完了時にもう一度計測し、トレンドとして継続するか確認する。
