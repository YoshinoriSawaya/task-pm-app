...
|---|---|---|
| BAC | 16.0h | - |
| PV | 15.0h(環境構築確認0.5h+バックエンド実装3.5h+フロントエンド実装3.5h+統合・E2Eテスト1.0h+リファクタリング・設計比較3.0h+デプロイ2.0h+PMBOKドキュメント整備1.5hが計画上完了しているべき時点) | 2026-09-06 |
| EV | 15.0h(全7フェーズがクローズ済み。**プロジェクト完了**) | 2026-09-06 |
| AC | **14.3h**(自己申告。#12+#17=8.0h、#22=1.0h、#25=1.0h、#29=2.0h、#34=2.0h、#40=0.3h) | 2026-09-06 |
| CV | **+0.7h**(EV-AC。プロジェクト完了時点で見積りを上回るペースに転じた) | 2026-09-06 |
| SV | 0.0h(EV=PVで計画通り) | 2026-09-06 |
| CPI | **1.05**(EV/AC。フェーズ8の大幅な前倒しにより初めて1.0を超えた) | 2026-09-06 |
| SPI | 1.0(全フェーズを計画順序通り完走) | 2026-09-06 |
| 最終実績 | **AC=14.3h**(BAC 16.0hに対し1.7h残して完了)。プロジェクトは完了しているため、以降のEAC/ETC/VACによる予測は行わない(進行中プロジェクト向けの指標であり、完了時点では実績ACそのものが最終値となる) | 2026-09-06 |

**最終総括**: フェーズ3完了時点のCPI=0.5(見積りの2倍のペース)から、Must/Should仕分け([change-log.md](change-log.md) C12)を境に0.83→0.85→0.96→0.96→**プロジェクト完了時点1.05**と、6回連続で改善し続け、最終的に見積りを上回るペースで完走した。Should分(#47〜#49)を含む全スコープを一切削らずに、BAC 16.0hに対しAC 14.3h(1.7h残し)で完了。「CPI低下は失敗ではなくスコープ判断のタイミングを示すシグナル」という[change-log.md](change-log.md) C12での方針転換が、最終的にプロジェクト全体の成功に直結したことが、6フェーズ分のデータで裏付けられた。
...
| デプロイ(#34) | 2.0h | **2.0h**(自己申告) | 見積り通り。IAM/Budgets等の金額セーフティ構築、EC2構築・障害対応(t3.micro→t3.small)、`php artisan serve`環境変数バグの切り分け・修正、nginx Basic認証実装、本番動作確認、リソース削除までを含む。CPI=**0.96**(前回と同水準を維持) |
| PMBOKドキュメント整備・振り返り(#40) | 1.5h | **0.3h**(20分、自己申告) | 見積りの5分の1で完了。change-log/risk-register/retrospective/READMEの4点はいずれも「フェーズごとに都度記録する」運用を最後まで徹底していたため、フェーズ8での作業は主に総括・最終整理のみで済んだ |
