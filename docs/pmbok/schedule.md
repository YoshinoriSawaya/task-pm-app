...
|---|---|---|
| BAC | 16.0h | - |
| PV | 7.5h(環境構築確認0.5h+バックエンド実装3.5h+フロントエンド実装3.5hが計画上完了しているべき時点) | 2026-09-06 |
| EV | 7.5h(同上3フェーズがクローズ済み。フロントエンドはMust分のみで#22の見積り3.5hを充足済みと評価。Should分の#47〜#49は元々この見積りに含まれない追加スコープのため対象外) | 2026-09-06 |
| AC | **9.0h**(自己申告。#12+#17=8.0h、#22=1.0h) | 2026-09-06 |
| CV | **-1.5h**(EV-AC。出来高よりコストがやや超過) | 2026-09-06 |
| SV | 0.0h(EV=PVで計画通り) | 2026-09-06 |
| CPI | **0.83**(EV/AC。前回0.5から改善) | 2026-09-06 |
| SPI | 1.0(フェーズの順序・範囲は計画通り) | 2026-09-06 |
| EAC | **19.2h**(AC+(BAC-EV)/CPI。前回32.0hから改善) | 2026-09-06 |
| ETC | **10.2h**(EAC-AC。残作業の見積り) | 2026-09-06 |
| VAC | **-3.2h**(BAC-EAC。前回-16.0hから改善) | 2026-09-06 |

**現状の読み方**: 前回(フェーズ3完了時点)のCPI=0.5から**CPI=0.83へ改善**した。ユーザーへのヒアリングにより、環境構築確認+バックエンド実装の実績8.0hのうち「実装自体は2時間ぐらい」との申告があり、残り約6時間の大半はPHPツールチェイン更新・Composer依存解決トラブルという**一過性の環境構築コスト**([retrospective.md](retrospective.md)フェーズ2)だったと推測される。フロントエンド実装(Must分)が見積り3.5hに対し実績1.0hと大幅に上回るペースで完了したことが、この推測を裏付けている。**環境構築という初期の固定費を払い終えた後は、TDDでの実装速度自体は見積りより速い**、という読み方ができる。とはいえサンプル数はまだ少なく、次フェーズ(統合・E2Eテスト)以降も継続して計測し、トレンドとして定着するか確認する。
